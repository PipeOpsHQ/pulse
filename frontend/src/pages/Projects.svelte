<script>
  ...
  async function handleUpdateQuota() {
    ...
    try {
      await api.patch(`/projects/${selectedProject.id}/quota`, {
        max_events_per_month: editingQuota,
      });
...
<div class="space-y-6 sm:space-y-8">
  ...
  >
    <div>
      ...
      <p class="text-sm text-slate-400">
        Manage your monitoring targets and quotas
      </p>
    </div>
    <button
  ...
  {:else if filteredProjects.length === 0}
    <div
      class="pulse-card flex flex-col items-center justify-center py-20 text-center"
    >
    ...
    >
      {#each filteredProjects as project}
        <div
          ...
          on:keydown={(e) =>
            e.key === "Enter" && navigate(`/projects/${project.id}`)}
        ...
        >
          <div class="mb-3 flex items-start justify-between">
            <div
              class="flex h-8 w-8 items-center justify-center rounded-lg bg-pulse-500/10 text-pulse-400"
            >
              <Activity size={18} />
            </div>
          ...
          <div class="flex items-center justify-between mb-2.5">
            <h3
              class="text-xs font-semibold text-white group-hover:text-pulse-400 transition-colors truncate pr-2"
            >
              {project.name}
            </h3>
            <span
          ...
          <div class="space-y-3">
            ...
            <div>
              <div class="flex items-center justify-between text-[10px] mb-1">
                ...
                <span
                  class="font-medium {project.current_month_events >=
              ...
              <div class="h-1 w-full overflow-hidden rounded-full bg-white/5">
                <div
                  class="h-full rounded-full transition-all duration-500 {project.current_month_events >=
                  project.max_events_per_month
                    ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]'
                    : 'bg-pulse-500 shadow-[0_0_8px_rgba(139,92,246,0.4)]'}"
            ...
            <div
              class="flex items-center justify-between text-[9px] text-slate-600 pt-2 border-t border-white/5"
            >
              <div class="flex items-center gap-1">
                <Hash size={10} />
              ...
              <div
                class="flex items-center gap-1 text-pulse-500 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <span>DETAILS</span>
...
{#if showCreateModal}
  <div class="fixed inset-0 z-[2000] flex items-center justify-center p-4">
    ...
    ></button>
    <div
      class="pulse-card relative w-full max-w-md border-white/10 p-6 sm:p-8 shadow-2xl"
    >
      <h2 class="text-xl sm:text-2xl font-bold text-white mb-2">
        Create New Project
      </h2>
      <p class="text-xs sm:text-sm text-slate-400 mb-6">
        Enter a name for your monitoring target
      </p>
...
{#if showQuotaModal && selectedProject}
  <div class="fixed inset-0 z-[2000] flex items-center justify-center p-4">
    ...
    ></button>
    <div
      class="pulse-card relative w-full max-w-sm border-white/10 p-6 sm:p-8 shadow-2xl"
    >
      <h2 class="text-xl sm:text-2xl font-bold text-white mb-2">
        Adjust Quota
      </h2>
      <p class="text-xs sm:text-sm text-slate-400 mb-6">
        Set the monthly event limit for <span class="text-pulse-400 font-bold"
      ...
      <div class="space-y-4">
        <div>
          ...
          <div class="relative">
            ...
            />
            <span
              class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-600 uppercase"
              >MONTH</span
            >
          </div>
          <p class="mt-2 text-[10px] text-slate-500 leading-tight">
            Once this limit is reached, incoming events will be rejected with a
            429 status code.
          </p>
        </div>
