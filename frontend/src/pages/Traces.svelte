...
<div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 sm:p-6 lg:p-8">
  <div class="mx-auto max-w-7xl">
    <div class="mb-8">
      ...
      <p class="text-slate-400">View and analyze performance traces for your project</p>
    </div>
    ...
    {:else if traces.length === 0}
      <div class="pulse-card p-12 text-center">
        ...
        <p class="text-sm text-slate-500 mt-2">Traces will appear here when your application sends performance data</p>
      </div>
    {:else}
      <div class="space-y-4">
        {#each traces as trace}
          <Link to="/projects/{projectId}/traces/{trace.trace_id}" class="block">
              ...
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <span class="font-mono text-sm text-pulse-400 truncate">{trace.trace_id}</span>
                  </div>

                  {#if trace.name}
                    <h3 class="text-lg font-semibold text-white mb-2 group-hover:text-pulse-400 transition-colors">
                      {trace.name}
                    </h3>
                  {/if}

                  {#if trace.description}
                    <p class="text-sm text-slate-400 mb-4">{trace.description}</p>
                  {/if}
                  ...
                  <div class="flex flex-wrap items-center gap-4 text-xs text-slate-500">
                    ...
                    {#if trace.start_timestamp && trace.timestamp}
                      <div class="flex items-center gap-1.5">
                        <Clock size={12} />
                        <span>{formatDuration(trace.start_timestamp, trace.timestamp)}</span>
                      </div>
                    {/if}

                    {#if trace.status}
                      <span class="rounded px-2 py-0.5 bg-white/10 text-slate-300 uppercase text-[10px] font-bold">
                        {trace.status}
                      </span>
                    {/if}
                  </div>
                ...
                <ChevronRight size={20} class="text-slate-600 group-hover:text-pulse-400 transition-colors shrink-0" />
              </div>
            </div>
          </Link>
        {/each}
