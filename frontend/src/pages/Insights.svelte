  ...
  {:else if insights}
    ...
    >
      <div>
        ...
        <p class="text-xs text-slate-400">
          Comprehensive overview of errors, traces, uptime, and coverage
        </p>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <!-- Time Range Filter -->
        <div
          class="flex items-center gap-1 rounded-lg border border-white/[0.08] bg-white/[0.04] p-1"
        >
          <button
            on:click={() => {
              timeRange = "24h";
            ...
            class="px-3 py-1.5 text-xs font-medium rounded transition-all {timeRange ===
            '24h'
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-2xl font-bold text-white">
                {formatNumber(insights.errors?.total_errors || 0)}
              </div>
            </div>
          </div>
        </div>
        <div class="space-y-2">
          <div class="flex items-center justify-between text-xs">
            ...
            <span class="font-bold text-white"
              >{formatNumber(insights.errors?.by_status?.unresolved || 0)}</span
            >
          </div>
          <div class="flex items-center justify-between text-xs">
            <span class="text-slate-500">Resolved</span>
            <span class="font-bold text-emerald-400"
              >{formatNumber(insights.errors?.by_status?.resolved || 0)}</span
            >
          </div>
        </div>
      </div>
        ...
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-2xl font-bold text-white">
                {formatNumber(insights.traces?.total_traces || 0)}
              </div>
            </div>
          </div>
        </div>
        <div class="space-y-2">
          <div class="flex items-center justify-between text-xs">
            ...
            <span class="font-bold text-white font-mono"
              >{formatDuration(insights.traces?.avg_duration_ms || 0)}</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="pulse-card p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-2xl font-bold text-white">
                {formatPercentage(insights.uptime?.avg_uptime_7d || 0)}
              </div>
            </div>
          </div>
        </div>
        <div class="space-y-2">
          <div class="flex items-center justify-between text-xs">
            ...
            <span class="font-bold text-white"
              >{formatNumber(insights.uptime?.total_monitors || 0)}</span
            >
          </div>
          <div class="flex items-center justify-between text-xs">
            <span class="text-slate-500">24h Uptime</span>
            <span class="font-bold text-emerald-400"
              >{formatPercentage(insights.uptime?.avg_uptime_24h || 0)}</span
            >
          </div>
        </div>
      </div>
        ...
        <div class="space-y-2">
          {#if insights.coverage?.trend}
            ...
            {@const trendColor = getTrendColor(
              insights.coverage.trend.direction,
            )}
            <div class="flex items-center justify-between text-xs">
          ...
          <div class="flex items-center justify-between text-xs">
            ...
            <span class="font-bold text-white"
              >{formatNumber(insights.coverage?.history_count || 0)}</span
            >
          </div>
        </div>
      </div>
        ...
        {#if insights.errors?.by_level}
          <div class="space-y-3">
            {#each Object.keys(insights.errors.by_level || {}) as level}
              ...
              {@const percentage =
                insights.errors.total_errors > 0
                  ? (count / insights.errors.total_errors) * 100
                  : 0}
              <div>
                <div class="flex items-center justify-between mb-1.5">
                  ...
                  <span class="text-xs font-bold text-white"
                    >{formatNumber(count)} ({percentage.toFixed(1)}%)</span
                  >
                </div>
                <div
                  class="h-1.5 w-full rounded-full bg-white/5 overflow-hidden"
                >
                  <div
                    class="h-full rounded-full transition-all duration-500 {level ===
                      'error' || level === 'fatal'
                      ? 'bg-red-500'
                      : level === 'warning'
                        ? 'bg-amber-500'
        ...
        <div class="space-y-4">
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              ...
              <span class="text-sm font-bold text-white"
                >{formatPercentage(insights.uptime?.avg_uptime_7d || 0)}</span
              >
            </div>
            <div class="h-2 w-full rounded-full bg-white/5 overflow-hidden">
              <div
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              ...
              <span class="text-sm font-bold text-white"
                >{formatPercentage(insights.uptime?.avg_uptime_30d || 0)}</span
              >
            </div>
            <div class="h-2 w-full rounded-full bg-white/5 overflow-hidden">
              <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-3">
          {#if insights.errors?.recent && insights.errors.recent.length > 0}
            {#each insights.errors.recent.slice(0, 5) as error}
              ...
              >
                <div
                  class="h-2 w-2 rounded-full {error.level === 'error' ||
                  error.level === 'fatal'
                    ? 'bg-red-500'
                    : error.level === 'warning'
                      ? 'bg-amber-500'
                ...
                <div class="flex-1 min-w-0">
                  <div
                    class="text-xs font-semibold text-white truncate group-hover:text-pulse-400 transition-colors"
                  >
      ...
      <div class="pulse-card p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          {#if selectedProject}
            <Link
              to="/projects/{selectedProject}/traces"
              class="text-xs font-bold text-pulse-400 hover:text-white transition-colors"
            >
              View All →
            </Link>
        ...
        <div class="space-y-3">
          {#if insights.traces?.recent && insights.traces.recent.length > 0}
            {#each insights.traces.recent.slice(0, 5) as trace}
              {@const duration =
                trace.start_timestamp && trace.timestamp
                  ? new Date(trace.timestamp).getTime() -
                    new Date(trace.start_timestamp).getTime()
                  : 0}
              <div
                class="flex items-center gap-3 p-3 rounded-lg border border-white/[0.05] bg-white/[0.02]"
              >
    ...
    {#if insights.coverage?.recent_history && insights.coverage.recent_history.length > 0}
      <div class="pulse-card p-6 mt-6">
        ...
        <div
          class="h-32 w-full rounded-lg bg-black/40 p-4 border border-white/5"
        >
          <svg viewBox="0 0 400 100" class="h-full w-full overflow-visible">
            {#if insights.coverage.recent_history && insights.coverage.recent_history.length > 0}
              {@const points = insights.coverage.recent_history
                .map((h, i) => {
                  const x =
                    (i / (insights.coverage.recent_history.length - 1)) * 380 +
                    10;
      ...
      <div class="pulse-card p-6 mt-6">
        ...
        <div class="space-y-3">
          {#each insights.uptime.monitors as monitor}
            <div
              class="flex items-center justify-between p-3 rounded-lg border border-white/[0.05] bg-white/[0.02]"
            >
              <div class="flex-1 min-w-0">
                <div class="text-xs font-semibold text-white truncate">
                  {monitor.name}
                ...
                <div
                  class="text-[10px] text-slate-500 mt-0.5 font-mono truncate"
                >
                  {monitor.url}
                </div>
              </div>
