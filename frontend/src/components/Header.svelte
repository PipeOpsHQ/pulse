...
<header
  class="sticky top-0 z-50 h-16 border-b border-white/[0.08] bg-black/40 backdrop-blur-2xl shadow-[0_1px_0_rgba(255,255,255,0.05)]"
>
  <div
    class="mx-auto flex h-full w-full max-w-7xl items-center justify-between px-6"
  >
    <div class="flex items-center gap-4">
      ...
      <nav
        class="flex flex-1 items-center gap-2 text-sm overflow-x-auto no-scrollbar whitespace-nowrap pr-4"
    ...
    <div class="flex items-center gap-6">
      {#if showSearch}
        <div class="group relative hidden sm:flex items-center">
          <Search
            size={16}
            class="absolute left-3 text-slate-500 transition-colors group-focus-within:text-pulse-500"
          />
          <input
          ...
          />
          <div
            class="absolute right-2 flex items-center gap-1 rounded border border-white/10 bg-black/40 px-1.5 py-0.5 text-[10px] font-medium text-slate-500"
          >
            <Command size={10} />
            <span>K</span>
          </div>
